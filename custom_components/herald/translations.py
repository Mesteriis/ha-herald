...
RUNTIME_FEEDBACK = {
    "ru": {
        "maintenance_on": "Геральд перешел в режим обслуживания. Уведомления ниже уровня {level} временно отключены.",
        "maintenance_off": "Геральд вышел из режима обслуживания. Обычная доставка уведомлений восстановлена.",
        "mute_all_on": "Геральд включил глобальное беззвучие. Все уведомления ниже critical временно заглушены.",
        "mute_all_off": "Геральд отключил глобальное беззвучие. Обычная доставка уведомлений восстановлена.",
    },
    "en": {
...
RU_NOTIFICATION_TOKEN_REPLACEMENTS: tuple[tuple[str, str], ...] = (
    ("AI summary unavailable", "Сводка AI недоступна"),
    ...
    ("exceptional", "необычные погодные условия"),
    ("lightning", "гроза"),
    ("cloudy", "облачно"),
    ("pouring", "ливень"),
    ("rainy", "дождь"),
...
def normalize_notification_text(text: str, language: str) -> str:
    """Apply small language-specific fixes to notification text."""
    if not text:
        return text
    normalized = str(text)
    if language != "ru":
    ...
    normalized = re.sub(r"\bHerald\b", "Геральд", normalized)
    for source, target in RU_NOTIFICATION_TOKEN_REPLACEMENTS:
        normalized = re.sub(
    ...
    normalized = re.sub(r"\bN/A\b", "н/д", normalized, flags=re.IGNORECASE)
...
def runtime_control_change_text(
    *,
    ...
    if key == "dashboard_sidebar":
        return _by_language(
            language,
            ru=f"Геральд: dashboard в боковой панели {enabled_text}.",
    ...
    if key == "ai_enabled":
        return _by_language(
            language,
            ru=f"Геральд: глобальный AI {enabled_text}.",
    ...
    if key.startswith("ai_level:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: AI для уровня {levels.get(level, level)} {enabled_text}.",
    ...
    if key.startswith("level_enabled:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: уровень {levels.get(level, level)} {enabled_text}.",
    ...
    if key.startswith("channel_family:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: семейство каналов {family} {enabled_text}.",
    ...
    if key == "maintenance_min_level":
        ...
        return _by_language(
            language,
            ru=f"Геральд: минимальный уровень обслуживания {level}.",
    ...
    if key.startswith("room_presence:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: fallback-присутствие комнаты {room} {enabled_text}.",
    ...
    if key.startswith("room_audio_target:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: устройство уведомлений для комнаты {room} переключено на {target}.",
    ...
    if key.startswith("user_language:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: язык пользователя {user} переключен на {target_language}.",
    ...
    if key.startswith("user_character:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: персонаж пользователя {user} переключен на {value}.",
    ...
    if key.startswith("user_silent:"):
        ...
        return _by_language(
            language,
            ru=f"Геральд: тихий режим пользователя {user} {enabled_text}.",
