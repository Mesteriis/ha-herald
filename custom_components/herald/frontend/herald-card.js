...
var HeraldCard = class extends i4 {
  ...
  render() {
    ...
    if (!today || !last || !queue) {
      return b2`<ha-card><div class="shell">Herald entities are not available.</div></ha-card>`;
    }
    ...
    return b2`
      <label class="language-card">
        ...
        <select .value=${entity.state} @change=${(event) => this._setLanguage(entityId, event)}>
          ${options.map((option) => b2`<option value=${option}>${option}</option>`)}
        </select>
      </label>
    `;
  }
