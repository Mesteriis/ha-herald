...
class HeraldFrontendRegistration:
    ...
    def _prefer_yaml_dashboard(self) -> bool:
        """Return True when a dedicated YAML Herald dashboard already exists."""
        panels = self.hass.data.get(DATA_PANELS, {})
        if "lovelace-herald" in panels:
            return True
        try:
            return Path(self.hass.config.path("dashboards/herald_dashboard.yaml")).exists()
    ...
    async def _async_register_lovelace_resource(self) -> bool:
        ...
        try:
            return any(
                str(resource.get("url", "")) == FRONTEND_MODULE_URL
        ...
        for resource in existing:
            ...
            if callable(update_item):
                try:
                    ...
                    _LOGGER.debug(
                        "Failed to update Herald Lovelace resource %s -> %s: %s",
                        resource_url,
                        FRONTEND_MODULE_URL,
                        err,
                    )
    ...
    async def _async_remove_storage_dashboard(self) -> None:
        """Drop the auto-installed storage dashboard when a YAML Herald panel exists."""
        dashboards = self._lovelace_value("dashboards")
        if isinstance(dashboards, dict):
            dashboards.pop(FRONTEND_DASHBOARD_URL_PATH, None)
        panels = self.hass.data.get(DATA_PANELS, {})
        panels.pop(FRONTEND_DASHBOARD_URL_PATH, None)
        remove_panel = getattr(frontend, "async_remove_panel", None)
        if callable(remove_panel):
            try:
                remove_panel(self.hass, FRONTEND_DASHBOARD_URL_PATH)
            except Exception:  # noqa: BLE001
                pass
        await self._async_remove_dashboard_registry_entry(FRONTEND_DASHBOARD_URL_PATH)
    ...
    async def _async_remove_dashboard_registry_entry(self, dashboard_id: str) -> None:
        """Remove one storage dashboard entry from the Lovelace registry."""
        store: Store[dict[str, Any]] = Store(
            self.hass,
            _LOVELACE_DASHBOARDS_STORAGE_VERSION,
            _LOVELACE_DASHBOARDS_STORAGE_KEY,
        ...
        data = await store.async_load() or {"items": []}
        items = [entry for entry in data.get("items", []) if entry.get("id") != dashboard_id]
        await store.async_save({"items": items})
