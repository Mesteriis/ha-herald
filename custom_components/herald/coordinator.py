...
from .const import (
    ...
    FRONTEND_MODULE_URL,
    NOTIFICATION_POLICIES_STORAGE_KEY,
    NOTIFICATION_REGISTRY_STORAGE_KEY,
...
class HeraldCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(
        ...
        self._notification_registry_store: Store[dict[str, Any]] = Store(
            hass,
            STORAGE_VERSION,
            f"{NOTIFICATION_REGISTRY_STORAGE_KEY}.{entry.entry_id}",
        )
        self._notification_policies_store: Store[dict[str, Any]] = Store(
            hass,
            STORAGE_VERSION,
            f"{NOTIFICATION_POLICIES_STORAGE_KEY}.{entry.entry_id}",
        )
        ...
        self.notification_policies = NotificationPolicyManager(Path(hass.config.path("")))
        ...
        self._notification_registry_tick_unsub: Any | None = None
        self._control_snapshot: dict[str, Any] = {}
        self._topology_snapshot: dict[str, Any] = {}
        self._notification_registry: dict[str, dict[str, Any]] = {}
        self._notification_policy_overrides: dict[str, dict[str, Any]] = {}
        self._notification_registry_refreshed_at: str | None = None

    async def _async_setup(self) -> None:
        """Load persistent state and augment autodiscovered channels."""
        if stored := await self._store.async_load():
            self._state = RuntimeState.from_dict(stored)
        if stored_registry := await self._notification_registry_store.async_load():
            self._notification_registry = dict(stored_registry.get("items", {}))
    ...
    async def _async_prepare_service_context(
        self,
        data: dict[str, Any],
        ...
        return request, runtime_context, flow_name, flow, context
    ...
    async def async_refresh_notification_registry(self, *, force: bool = False) -> dict[str, Any]:
        """Rescan the active configuration and rebuild the derived notification registry."""
        now = dt_util.now()
        if not force and self._notification_registry_refreshed_at:
            try:
                refreshed_at = dt_util.parse_datetime(self._notification_registry_refreshed_at)
            except Exception:  # noqa: BLE001
                refreshed_at = None
            if refreshed_at is not None and (now - refreshed_at) < timedelta(minutes=10):
        ...
        try:
            ...
            self._append_trace(
                {
                    "stage": "notification_registry_refresh",
                    "count": len(self._notification_registry),
                    "force": force,
                    "status": "error",
                    "error": str(err),
                }
        ...
        self._notification_registry = dict(registry)
        self._notification_registry_refreshed_at = now.isoformat()
        await self._notification_registry_store.async_save(
            {
                "refreshed_at": self._notification_registry_refreshed_at,
                "items": self._notification_registry,
            }
        )
        self._append_trace(
            {
    ...
    def notification_registry_snapshot(self) -> dict[str, Any]:
        """Return the derived registry together with effective user policy."""
        effective = self.notification_policies.effective_registry(
            registry=self._notification_registry,
    ...
    def notification_registry_summary(self) -> dict[str, Any]:
        """Return a compact summary for dashboard and sensor attributes."""
        snapshot = self.notification_registry_snapshot()
        items = list(snapshot.get("items", []))
        enabled_count = sum(1 for item in items if bool(item.get("effective", {}).get("enabled", True)))
    ...
    def get_notification_policy(self, notification_key: str) -> dict[str, Any]:
        """Return one merged notification policy payload."""
        entry = dict(self._notification_registry.get(notification_key, {}))
        ...
        return {
            **entry,
            "notification_key": notification_key,
            "policy": NotificationPolicyOverride.from_dict(
    ...
    async def async_set_notification_policy(self, payload: dict[str, Any]) -> dict[str, Any]:
        """Create or update one user policy override."""
        ...
        if notification_key not in self._notification_registry:
            await self.async_refresh_notification_registry(force=True)
        if notification_key not in self._notification_registry:
        ...
        await self._notification_policies_store.async_save({"items": self._notification_policy_overrides})
        self._append_trace(
            {
                "stage": "notification_policy_set",
                "notification_key": notification_key,
                "policy": current.to_dict(),
            }
        )
        await self.async_request_refresh()
        return self.get_notification_policy(notification_key)

    async def async_reset_notification_policy(self, notification_key: str) -> dict[str, Any]:
        """Remove one user policy override and fall back to inherited routing."""
        key = str(notification_key).strip()
        self._notification_policy_overrides.pop(key, None)
        await self._notification_policies_store.async_save({"items": self._notification_policy_overrides})
        self._append_trace(
            {
                "stage": "notification_policy_reset",
                "notification_key": key,
            }
        )
        await self.async_request_refresh()
        return self.get_notification_policy(key)
    ...
    async def _async_refresh_runtime_sources(self) -> None:
        ...
        await self.async_refresh_notification_registry()
    ...
    def _build_snapshot(self, presence) -> dict[str, Any]:
        ...
        return {
            ...
            "notification_registry_summary": self.notification_registry_summary(),
            "notification_registry": self.notification_registry_snapshot(),
            "notification_policies": dict(self._notification_policy_overrides),
    ...
    def _build_topology_snapshot(self, presence) -> dict[str, Any]:
        """Build a UI-friendly topology view of rooms, users, and channels."""
        room_control_entities = set(self._control_summary().get("rooms", []))
        rooms: list[dict[str, Any]] = []
        for room_name, sensor_entity in sorted(self.presence.room_sensors().items()):
            fallback_entity = room_presence_entity_id(room_name)
            audio_target_entity = room_audio_target_entity_id(room_name)
            voice_targets: list[dict[str, Any]] = []
            tv_targets: list[dict[str, Any]] = []
            attached_channels: list[str] = []
            for channel in self.config.channels.values():
                channel_rooms = self._channel_topology_rooms(channel)
                if room_name not in channel_rooms:
                    continue
                attached_channels.append(channel.name)
                if channel.channel_type in {"tts", "tts_hume"}:
                    raw_targets = channel.data.get("audio_targets", {}).get(room_name, [])
                    if isinstance(raw_targets, list):
                        for target in raw_targets:
                            entity_id = str(target.get("entity_id") or "").strip()
                            voice_targets.append(
                                {
                                    "channel": channel.name,
                                    "kind": str(target.get("kind") or "voice"),
                                    "entity_id": entity_id,
                                    "service": str(target.get("service") or channel.service or ""),
                                    "available": self._entity_available(entity_id, active_only=bool(target.get("active_only", False))),
                                }
                            )
                if channel.channel_type == "tv":
                    entity_id = str(
                        dict(channel.data.get("room_targets", {})).get(room_name)
                        or channel.entity_id
                        or ""
                    ).strip()
                    service = str(
                        dict(channel.data.get("notify_services", {})).get(room_name)
                        or channel.service
                        or ""
                    ).strip()
                    tv_targets.append(
                        {
                            ...
                            "entity_id": entity_id,
                            "service": service,
                            "available": self._entity_available(
                                entity_id,
            ...
            rooms.append(
                {
                    ...
                    "occupied": room_name in list(presence.occupied_rooms),
                    "primary": room_name == presence.primary_room,
                    "presence_entity": sensor_entity,
                    "fallback_entity": fallback_entity,
                    "audio_target_entity": audio_target_entity if audio_target_entity in room_control_entities else None,
                    "audio_target_preference": self.controls.room_audio_target(room_name, default="auto"),
                    "channels": sorted(set(attached_channels)),
                    "voice_targets": voice_targets,
                    "tv_targets": tv_targets,
                }
            )
        ...
        if async_all is not None:
            person_states = list(async_all("person"))
        person_map = {state.entity_id.split(".", maxsplit=1)[1]: state for state in person_states}
        users: list[dict[str, Any]] = []
        for slug in sorted({*self.config.users.keys(), *person_map.keys()}):
            person_state = person_map.get(slug)
            config_user = self.config.users.get(slug)
            users.append(
                {
                    "slug": slug,
                    "name": (
                        str(person_state.attributes.get("friendly_name"))
                        if person_state is not None and person_state.attributes.get("friendly_name")
                        else (config_user.name if config_user is not None else self._humanize_slug(slug))
                    ),
                    "person_entity_id": f"person.{slug}" if person_state is None else person_state.entity_id,
                    "home": bool(person_state is not None and person_state.state == "home"),
                    "language": self.controls.user_language(slug, default="ru"),
                    "character": self.controls.user_character(slug, default=DEFAULT_PERSONALITY),
                    "silent": self.controls.user_silent(slug),
                    "presence_entity": config_user.presence_entity if config_user is not None else None,
                    "room_entity": config_user.room_entity if config_user is not None else None,
                }
            )
        ...
        channels = [
            {
                ...
                "quiet_hours_policy": channel.quiet_hours_policy,
                "room": channel.room,
                "rooms": self._channel_topology_rooms(channel),
                "user": channel.user,
                "service": channel.service,
                "targets": self.router._iter_channel_entity_ids(channel),
            }
            for channel in sorted(self.config.channels.values(), key=lambda item: item.name)
        ]
        ...
        return {
            ...
            "primary_room": presence.primary_room,
        }

    @staticmethod
    def _channel_topology_rooms(channel: ChannelConfig) -> list[str]:
        """Return all rooms a channel is attached to for UI topology views."""
        rooms: set[str] = set()
        if channel.room:
            rooms.add(str(channel.room))
        rooms.update(str(room_name) for room_name in dict(channel.data.get("room_targets", {})))
        rooms.update(str(room_name) for room_name in dict(channel.data.get("audio_targets", {})))
        rooms.update(str(room_name) for room_name in dict(channel.data.get("notify_services", {})))
        return sorted(room for room in rooms if room)

    def _entity_available(self, entity_id: str, *, active_only: bool) -> bool:
        """Return whether the entity exists and is currently usable."""
        if not entity_id:
            return False
        ...
        inactive_states = {"off", "idle", "standby", "unavailable", "unknown"}
        unavailable_states = {"off", "unavailable", "unknown"}
        return state_value not in (inactive_states if active_only else unavailable_states)
    ...
    def _augment_runtime_config(self) -> None:
        ...
        if voice_auto is not None and voice_auto.quiet_hours_policy == QUIET_HOURS_POLICY_DEFAULT:
            voice_auto.quiet_hours_policy = QUIET_HOURS_POLICY_ALLOW
    ...
    def _apply_maintenance_routing(self, context: NotificationContext) -> None:
        """Redirect all notifications to HA tray-friendly channels during maintenance."""
        context.channels = ["persistent_default", "system_log_default"]
        context.rewrite = False
        context.summarize = False
        context.metadata = {
            **dict(context.metadata),
            "bypass_channel_policy": True,
            "maintenance_redirect": True,
            "maintenance_tray_debug": True,
    ...
    def _apply_notification_policy(self, notification_key: str, context: NotificationContext) -> str:
        ...
        context.metadata = {
            **dict(context.metadata),
            "notification_key": notification_key,
            "notification_policy": dict(effective),
        }
        if not bool(effective.get("enabled", True)):
            return "drop"
        if level_override := str(effective.get("level_override") or "").strip():
    ...
    def _register_event_listeners(self) -> None:
        ...
        self._notification_registry_tick_unsub = self._schedule_notification_registry_tick()

    def _schedule_notification_registry_tick(self):
        """Schedule the next background notification-registry refresh."""
        if self._notification_registry_tick_unsub is not None:
            self._notification_registry_tick_unsub()
        return async_call_later(
            self.hass,
            timedelta(minutes=15),
            self._async_handle_notification_registry_tick,
        )

    async def _async_handle_notification_registry_tick(self, _: Any) -> None:
        """Periodically rescan notification routes so the governance panel stays current."""
        try:
            await self.async_refresh_notification_registry(force=True)
            await self.async_request_refresh()
        finally:
    ...
    def _build_dashboard_payload(self, *, title: str, preset: str) -> dict[str, Any]:
        ...
        overview_cards: list[dict[str, Any]] = [
            ...
            },
            {
                ...
                "title": "Центр уведомлений Herald",
            },
            {
                "type": "grid",
                "columns": 4,
            ...
            },
            {
                ...
                "cards": [
                    {
                        "type": "markdown",
                        "title": "Маршрутизация сейчас",
                        "content": HeraldCoordinator._dashboard_routing_summary_markdown(),
                    },
                    {
                        "type": "entities",
                        "title": "Операции",
            ...
            {
                "type": "markdown",
                "title": "Последний предпросмотр маршрута",
                "content": HeraldCoordinator._dashboard_route_preview_markdown(),
            },
            {
                "type": "markdown",
                "title": "Очередь",
        ...
        routing_cards: list[dict[str, Any]] = [
            {
                "type": "markdown",
            ...
            {
                "type": "markdown",
                "title": "Комнаты и локальные цели",
                "content": HeraldCoordinator._dashboard_topology_rooms_markdown(),
            },
            {
                "type": "grid",
                "columns": 2,
                ...
                "cards": [
                    {
                        "type": "markdown",
                        "title": "Пользователи",
                        "content": HeraldCoordinator._dashboard_topology_users_markdown(),
                    },
                    {
                        "type": "markdown",
                        "title": "Каналы",
                        "content": HeraldCoordinator._dashboard_topology_channels_markdown(),
                    },
                ],
            },
        ]
                ...
                "cards": [
                    {
                        ...
                        "entities": [
                            {
                                "entity": entity_id,
                                "name": HeraldCoordinator._dashboard_system_entity_name(entity_id),
                            }
                            for entity_id in [
                                "switch.herald_ai_enabled",
                                "switch.herald_maintenance_mode",
                    ...
                    },
                    {
                        ...
                        "entities": [
                            ...
                            for entity_id in channel_family_entities
                        ],
                    },
                ],
            },
    ...
    @staticmethod
    def _dashboard_channel_label(value: str) -> str:
        """Return a readable delivery-channel label for dashboard UI."""
        labels = {
            ...
            "tv_default": "TV",
            "tv_auto": "TV: авто",
            "tv_living_room": "TV: гостиная",
            "mobile_aleksandrs_macbook_pro": "MacBook Pro Александра",
            "mobile_ipad": "iPad",
            "mobile_iphone_aleksander": "iPhone Александра",
            "mobile_iphone_vi": "iPhone Виктории",
            "mobile_macbook": "MacBook",
        }
        return labels.get(value, HeraldCoordinator._humanize_slug(value))

    @staticmethod
    def _dashboard_flow_label(value: str) -> str:
        """Return a readable flow label for dashboard UI."""
        labels = {
            "security_alerts": "Безопасность",
            "system_events": "Системные события",
            "device_alerts": "Устройства",
            "ai_events": "ИИ-события",
            "energy_events": "Энергособытия",
    ...
    @staticmethod
    def _dashboard_drop_reason_label(value: str) -> str:
        """Return a readable drop-reason label for dashboard UI."""
        labels = {
            "notification_policy_disabled": "Отключено политикой",
            "flow_disabled": "Поток выключен",
            "level_disabled": "Уровень выключен",
            "maintenance_redirect": "Режим обслуживания",
            "channel_disabled": "Канал выключен",
            "quiet_hours_blocked": "Тихие часы",
            "deduplicated": "Дедупликация",
            "cooldown_active": "Пауза доставки",
        }
        return labels.get(value, HeraldCoordinator._humanize_slug(value))

    @staticmethod
    def _dashboard_user_entity_name(entity_id: str) -> str:
        """Return a readable user-control label for generated dashboards."""
        object_id = entity_id.split(".", maxsplit=1)[-1]
        if object_id.endswith("_language"):
            return "Язык"
        if object_id.endswith("_character"):
            return "Персонаж"
        if object_id.endswith("_silent"):
            return "Без звука"
        return HeraldCoordinator._humanize_slug(object_id)

    @staticmethod
    def _dashboard_channel_entity_name(entity_id: str) -> str:
        """Return a readable channel-control label for generated dashboards."""
        object_id = entity_id.split(".", maxsplit=1)[-1]
        if object_id.endswith("_enabled"):
            return "Включено"
        if object_id.endswith("_min_level"):
            return "Мин. уровень"
        if object_id.endswith("_test"):
            return "Тест"
        return HeraldCoordinator._humanize_slug(object_id)

    @staticmethod
    def _dashboard_flow_entity_name(entity_id: str) -> str:
        """Return a readable flow-control label for generated dashboards."""
        object_id = entity_id.split(".", maxsplit=1)[-1]
        if object_id.endswith("_enabled"):
            return "Включено"
        if object_id.endswith("_summary_window"):
            return "Окно сводки"
        if object_id.endswith("_dedup_window"):
            return "Окно дедупликации"
        if object_id.endswith("_cooldown"):
            return "Пауза доставки"
        return HeraldCoordinator._humanize_slug(object_id)

    @staticmethod
    def _dashboard_system_entity_name(entity_id: str) -> str:
        """Return a readable system-control label for generated dashboards."""
        labels = {
            "switch.herald_ai_enabled": "ИИ включен",
            "switch.herald_maintenance_mode": "Режим обслуживания",
            "switch.herald_mute_all": "Глобальная тишина",
            "switch.herald_dashboard_sidebar": "Herald в сайдбаре",
            "switch.herald_ai_info": "ИИ для info",
            "switch.herald_ai_warning": "ИИ для warning",
            "switch.herald_ai_critical": "ИИ для critical",
            "switch.herald_level_info": "Уровень info",
            "switch.herald_level_warning": "Уровень warning",
            "switch.herald_level_critical": "Уровень critical",
            "select.herald_maintenance_min_level": "Мин. уровень обслуживания",
            "switch.herald_channel_voice": "Семейство голоса",
            "switch.herald_channel_push": "Семейство push",
            "switch.herald_channel_tv": "Семейство TV",
        }
        return labels.get(entity_id, HeraldCoordinator._humanize_slug(entity_id.split(".", maxsplit=1)[-1]))
        ...
        return [
            {
                ...
                "entities": [
                    {
                        "entity": entity_id,
                        "name": HeraldCoordinator._dashboard_user_entity_name(entity_id),
                    }
    ...
    def _dashboard_channel_cards(channel_entities: list[str]) -> list[dict[str, Any]]:
        ...
        return [
            {
                "type": "entities",
                "title": HeraldCoordinator._dashboard_channel_label(channel_name),
                "show_header_toggle": False,
                "state_color": True,
                "entities": [
                    {
                        "entity": entity_id,
                        "name": HeraldCoordinator._dashboard_channel_entity_name(entity_id),
                    }
                    for entity_id in sorted(
                        entities,
                        key=lambda entity_id: next(
    ...
    def _dashboard_flow_cards(flow_entities: list[str]) -> list[dict[str, Any]]:
        ...
        return [
            {
                "type": "entities",
                "title": HeraldCoordinator._dashboard_flow_label(flow_name),
                "show_header_toggle": False,
                "state_color": True,
                "entities": [
                    {
                        "entity": entity_id,
                        "name": HeraldCoordinator._dashboard_flow_entity_name(entity_id),
                    }
                    for entity_id in sorted(
                        entities,
                        key=lambda entity_id: next(
...
- **Тихие часы:** `{{{{ 'вкл' if quiet_hours else 'выкл' }}}}`
- **Режим обслуживания:** `{{{{ 'вкл' if maintenance else 'выкл' }}}}`
- **Очередь:** `{{{{ queue_size }}}}`"""

    @staticmethod
    def _dashboard_routing_summary_markdown() -> str:
...
{% set quiet_hours = state_attr(status_entity, 'quiet_hours') %}
- **Основная комната сейчас:** `{{ primary_room }}`
- **Занятые комнаты:** `{{ occupied_rooms|join(', ') if occupied_rooms else 'нет' }}`
- **Приоритет голоса:** `alisa -> homepod`
- **Политика TV:** только overlay, без TTS и без захвата контента
- **Политика мобильных каналов:** только для пользователей вне дома
- **Глобальная тишина:** `{{ 'вкл' if mute_all else 'выкл' }}` · **Обслуживание:** `{{ 'вкл' if maintenance else 'выкл' }}` · **Тихие часы:** `{{ 'вкл' if quiet_hours else 'выкл' }}`"""

    @staticmethod
    def _dashboard_route_preview_markdown() -> str:
        """Render the most recent route preview."""
        return """{% set status_entity = 'sensor.herald_notification_center_status' %}
{% set preview = state_attr(status_entity, 'last_route_preview') or {} %}
{% if preview %}
{% set requested = preview.requested or {} %}
{% set resolved = preview.resolved or {} %}
{% set prechecks = preview.prechecks or {} %}
{% set deliveries = preview.deliveries or [] %}
- **Событие:** `{{ requested.event or 'Herald' }}` · **Уровень:** `{{ requested.level or 'info' }}`
- **Запрошены комната/устройство:** `{{ requested.room or 'auto' }}` / `{{ requested.device or '-' }}`
- **Выбранная комната:** `{{ resolved.delivery_room or 'auto' }}`
- **Причина блокировки:** `{{ prechecks.blocked_reason or 'нет' }}`
- **Итоговые каналы:** `{{ (resolved.final_channels or [])|join(', ') if (resolved.final_channels or []) else 'нет' }}`
{% if deliveries %}
{% for item in deliveries %}
- **{{ item.channel }}** → `{{ item.type }}`{% if item.target_kind %} · `{{ item.target_kind }}`{% endif %}{% if item.target_entity_id %} · `{{ item.target_entity_id }}`{% endif %}{% if item.service %} через `{{ item.service }}`{% endif %} · `{{ item.status }}`
{% endfor %}
{% else %}
_Preview ещё не построил ни одного канала._
{% endif %}
{% else %}
_Сервис `herald.route_preview` ещё не вызывался._
{% endif %}"""

    @staticmethod
    def _dashboard_topology_rooms_markdown() -> str:
        """Render rooms, occupancy, and local targets."""
        return """{% set topology = state_attr('sensor.herald_notification_center_status', 'topology') or {} %}
{% set rooms = topology.rooms or [] %}
{% if rooms %}
{% for room in rooms %}
- **{{ room.label or room.room }}**: `{{ 'occupied' if room.occupied else 'idle' }}`{% if room.primary %} · `primary`{% endif %} · voice `{{ room.audio_target_preference or 'auto' }}`
  Каналы: `{{ (room.channels or [])|join(', ') if (room.channels or []) else 'none' }}`
{% if room.voice_targets %}
  Voice: {% for target in room.voice_targets %}`{{ target.kind }}` → `{{ target.entity_id or '-' }}`{% if not loop.last %}, {% endif %}{% endfor %}
{% endif %}
{% if room.tv_targets %}
  TV: {% for target in room.tv_targets %}`{{ target.entity_id or '-' }}` via `{{ target.service or '-' }}`{% if not loop.last %}, {% endif %}{% endfor %}
{% endif %}
{% endfor %}
{% else %}
_Комнаты пока не обнаружены._
{% endif %}"""

    @staticmethod
    def _dashboard_topology_users_markdown() -> str:
        """Render users and their routing-related runtime settings."""
        return """{% set topology = state_attr('sensor.herald_notification_center_status', 'topology') or {} %}
{% set users = topology.users or [] %}
{% if users %}
{% for user in users %}
- **{{ user.name or user.slug }}**: `{{ 'дома' if user.home else 'вне дома' }}` · язык `{{ user.language or 'ru' }}` · персонаж `{{ user.character or '-' }}` · без звука `{{ 'вкл' if user.silent else 'выкл' }}`
{% endfor %}
{% else %}
_Пользователи не обнаружены._
{% endif %}"""

    @staticmethod
    def _dashboard_topology_channels_markdown() -> str:
        """Render all known channels in a routing-oriented view."""
        return """{% set topology = state_attr('sensor.herald_notification_center_status', 'topology') or {} %}
{% set channels = topology.channels or [] %}
{% if channels %}
{% for channel in channels %}
- **{{ channel.name }}**: `{{ channel.type }}` · включен `{{ 'да' if channel.enabled else 'нет' }}` · мин. уровень `{{ channel.min_level or 'info' }}`
  {% if channel.rooms %}Комнаты: `{{ channel.rooms|join(', ') }}` · {% endif %}{% if channel.user %}Пользователь: `{{ channel.user }}` · {% endif %}{% if channel.service %}Сервис: `{{ channel.service }}`{% endif %}
{% endfor %}
{% else %}
_Каналы пока не обнаружены._
{% endif %}"""
...
{% if items %}
{% for item in items[:6] %}
- **{{ item.title or item.event or 'Лента панели' }}**: {{ item.message or '' }}
  `{{ item.level or 'info' }}` · `{{ labels.get(item.channel, item.channel or 'dashboard') }}` · {{ item.timestamp or '' }}
{% endfor %}
{% else %}
_Лента панели пока пуста._
{% endif %}"""
...
{% if items %}
{% for item in items[:8] %}
- **{{ item.title or item.event or 'Уведомление в очереди' }}**: {{ item.message or '' }}
